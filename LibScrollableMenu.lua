...
function ScrollableDropdownHelper:OnMouseEnter(control)
	...
	if tooltipData ~= nil then
		if type(tooltipData) == "function" then
			...
			local hasSubmenu = data.hasSubmenu
			if hasSubmenu == true then
				--Show the tooltip more to the top so the opening submenu does not overlay it
				-->BOTTOMLEFT at row's TOPRIGHT, x+scrollbar width, y-50 or similar
				InitializeTooltip(InformationTooltip, control, BOTTOMLEFT, ZO_SCROLL_BAR_WIDTH + 15, -25, TOPRIGHT)
			else
